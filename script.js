...
function stayWithMe(){
  ...
  const reply = pick(replies[style] || replies.english);
...
async function getRealAIReply(text, detectedStyle){
  ...
  if(!response.ok){
    ...
    throw new Error(data?.message || data?.error || "AI backend failed");
  }
...
async function sendMessage(){
  ...
  try{
    ...
    showTypingIndicator(false);
    appendBotMessage("AI connection me issue aa raha hai. Backend URL ya Gemini key check karni padegi.");
  }
}
/* =========================================================
