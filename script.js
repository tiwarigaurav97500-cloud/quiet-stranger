// 🔥 PERFORMANCE FIX — smooth UI + typing + autosave
let typingTimer;
...
function optimizedTyping(callback) {
  if (typingFrame) {
    cancelAnimationFrame(typingFrame);
  }

  typingFrame = requestAnimationFrame(() => {
    callback();
    typingFrame = null;
  });
}
...
function getMoodAdvice(mood){
  const adviceMap = {
    "😊 Good": [
      `Aaj aapka mood achha lg rha hai 💛
Iss feeling ko thoda enjoy kijiye.
• aaj ki ek achhi baat note kijiye
• apni pasand ka song suniye
• khud ko appreciate kijiye`,

      `Good mood ek small gift jaisa hota hai 🌞
Aaj pressure kam rkhiye aur is lightness ko feel kijiye.
• ek tiny win likhiye
• thoda fresh air lijiye
• kisi nice memory ko save kijiye`,

      `Aaj thoda halka feel ho raha hai, nice 💛
Isko overthink mat kijiye.
• apni progress notice kijiye
• ek peaceful kaam kijiye
• smile ko force mat kijiye, bss allow kijiye`
    ],

    "😐 Okay": [
      `Aaj ka mood bas okay hai, aur okay bhi enough hai.
Har din amazing feel karna zaroori nahi hota.
• paani pee lijiye
• 5 deep breaths lijiye
• ek chota easy task kijiye`,

      `Okay mood ka matlab failure nahi hota.
Kabhi-kabhi neutral rehna bhi body ka rest mode hota hai.
• thoda slow ho jaiye
• ek simple kaam choose kijiye
• khud par pressure mt daliye`,

      `Aaj bss normal sa din lag raha hai ☁️
Isme kuch galat nahi hai.
• body ko relax kijiye
• screen se thoda break lijiye
• ek soft thought likhiye`
    ],

    "😔 Low": [
      `Agar aap low feel kar rahi hain, to aaj softness zaroori hai.
Aaj khud se pyaar se baat karni hai.
• thoda rest lijiye
• jo feel ho raha hai usko dabaaiye mat
• next 10 minutes ko easy banaiye`,

      `Low feel hona weakness nahi hai 🫂
Kbhi-kbhi dil ko bas safe jagah chahiye hoti hai.
• ek slow breath lijiye
• blanket/pillow ke saath 2 min rest
• bas ek line likhiye: “abhi kya heavy hai?”`,

      `Aaj energy low hai to speed kam kar do.
Sab kuch aaj hi solve karna zaroori nahi.
• light music suno
• paani piyo
• apne aap se softly bolo: “main try kar rhi hoon”`
    ],

    "😣 Stressed": [
      `Agar aap stressed feel kar rhi hain, to pehle body ko calm krte hain.
Sab kuch ek saath solve karna zaroori nahi.
• jaw aur shoulders relax kijiye
• 4 sec inhale, 6 sec exhale
• sirf next small step choose kijiye`,

      `Stress ka matlab hai mind overload ho gaya hai 🌧️
Abhi full solution nhi, bss pressure kam krna hai.
• top 3 tensions likhiye
• sabse urgent ek choose kijiye
• baaki ko temporarily side rakhiye`,

      `Abhi body ko signal do ki danger nahi hai.
Slow hona allowed hai.
• screen se 30 sec door dekho
• hands relax karo
• ek kaam ko 5-minute task me tod do`
    ]
  };

  return randomFromList(adviceMap[mood] || [`Aaj aap jo bhi feel kar rahi hain, wo important hai.`]);
}
function getMoodActions(mood){
  const actions = {
    "😊 Good": [
      `Try this:
• aaj ki ek achi memory save kijiye
• kisi ko silently thanks feel kijiye
• apne aap ko boliye: “aaj maine acha handle kiya”`,

      `Good mood task:
• ek choti photo ya note save karo
• apni ek progress likho
• 5 min ke liye favourite song suno`,

      `Soft celebration:
• khud ko credit do
• aaj ka best moment likho
• kisi ek cheez ke liye grateful feel karo`
    ],

    "😐 Okay": [
      `Gentle reset:
• seedha baith jaiye
• shoulders halka relax kijiye
...
• no extra pressure`,
      ...
      `Small balance:
• phone side rakho 1 min
• aankhon ko rest do
• ek line likho: “abhi mujhe kya chahiye?”`
    ],

    "😔 Low": [
      `Soft care:
• blanket ya pillow ke saath 2 min rest
• ek line likhiye: “abhi sabse heavy kya lag raha hai?”
• aaj strong dikhna zaroori nahi hai`,

      `Low mood support:
• slow breathing karo
• apne favourite soft song ka 1 part suno
• khud ko blame mat karo`,

      `Tiny comfort:
• light off/soft light rakho
• paani piyo
• ek safe memory ya note open karo`
    ],

    "😣 Stressed": [
      `Stress release:
• screen se 1 min nazar hataiye
• hands aur shoulders shake out kijiye
• likhiye: abhi sabse zaroori ek kaam kya hai?
• baaki sab baad me`,

      `Pressure break:
• 4 sec inhale
• 6 sec exhale
• ek kaam ko 3 tiny steps me likho
• sirf first step karo`,

      `Mind unload:
• paper kholo
• saari tension points likho
• unme se sirf 1 urgent mark karo
• baaki ko kal/baad me shift karo`
    ]
  };

  return randomFromList(actions[mood] || [`Aap thoda slow ho jaiye.`]);
}
...
function renderMoodGraph(){
  ...
  if(last7.length === 0){
    graph.innerHTML = `<div class="mood-graph-empty">No mood data yet. Mood select karo, graph yahan dikhega.</div>`;
    return;
  }
  ...
  last7.forEach(([date, mood]) => {
    const wrap = document.createElement("div");
    wrap.className = "mood-bar-wrap";

    const emoji = document.createElement("div");
    emoji.className = "mood-bar-emoji";
    emoji.innerText = mood.split(" ")[0];

    const bar = document.createElement("div");
    bar.className = "mood-bar";
    bar.style.height = (moodScore[mood] || 50) + "%";
    bar.title = `${date} — ${mood}`;

    const dateLabel = document.createElement("div");
    dateLabel.className = "mood-bar-date";
    dateLabel.innerText = date.slice(5);
...
Bss abhi breathing par focus kijiye.`,
...
• nose se inhale
• mouth se exhale
• hrr exhale pe body ko thoda soften karo
...
• abhi thoda paani pee lijiye
• ek sip bhi enough hai
• body support milega to mood halka lg sakta hai`,

      `Hydration Pause 💧

• bottle/glass paas lao
• 3 slow sips lo
• har sip ke baad ek breath lo`,

      `Small care 💧

• paani piyo
• face relax karo
• khud se bolo: “main apna dhyan rkh rhi hoon”`
    ],

    stretch: [
      `Shoulder Relax 🫶

• shoulders upar uthaiye
• 3 sec hold
• dheere chhod dijiye
• neck halka left-right move kijiye`,

      `Quick Stretch 🫶

• dono hands stretch karo
• wrists rotate karo
• back seedhi karo
• jaw relax karo`,
...
• shoulders 5 baar roll karo
• hands shake karo
• ek deep breath lo
• body ko signal do: “safe”`
    ],
...
• 2 min kuch solve mat kijiye
• sirf settle hone dijiye`,

      `Music pause 🎵

• sirf ek calm track play karo
• lyrics pe pressure mt lo
• bss sound ko background me rehne do`,

      `Soft sound 🎵

• low volume
• slow breathing
• ek minute ke liye bss listen mode`
    ],

    kind: [
      `Kind Words 💛

“Aaj aap jo feel kar rhi ho, uske bawajood aap important hain.”
“Aapko aaj perfect hone ki zaroorat nhi hai.”
“Aap softness deserve krti hain.”`,

      `Soft reminder 💛

“Main abhi bhi try kar rhi hoon.”
“Slow progress bhi progress hai.”
“Ek hard moment meri poori life define nhi karta.”`,

      `Gentle self-talk 💛

“Abhi mushkil hai, pr mai akeli nhi hoon.”
“Mai apne aap se thoda pyaar se baat kr skti hoon.”
“Bss next small step.”`
    ]
  };

  box.innerText = randomFromList(actions[action] || [`Aap thoda slow ho jaiye.`]);
}
...
function saveDiaryPhoto(){
  ...
  reader.onload = function(e){
    try{
      ...
      renderDiaryPhotos();
      alert("File saved permanently in this folder 💛");
    } catch(err){
      alert("Storage full ho sakti hai. Chhoti file try karo ya purani files delete karo.");
      console.log(err);
    }
  };
...
function deleteDiaryPhoto(photoId){
  ...
  const ok = confirm("Delete this saved file from this diary folder?");
  if(!ok) return;
...
function renderDiaryPhotos(){
  ...
  if(!currentDiaryFolder){
    gallery.innerHTML = `<div class="diary-empty-box">Open a folder to see saved files.</div>`;
    return;
  }
  ...
  if(photos.length === 0){
    gallery.innerHTML = `<div class="diary-empty-box">No file memories in this folder yet.</div>`;
    return;
  }
  ...
  photos.forEach(photo => {
    ...
    if(photo.kind === "image"){
      ...
      img.src = photo.src;
      img.alt = photo.name || "Diary memory photo";
      img.loading = "lazy";
      ...
      vid.className = "diary-file-preview";
      card.appendChild(vid);
    } else if(photo.kind === "audio"){
      ...
      fileBox.className = "diary-file-box";
      fileBox.innerText = photo.kind === "pdf" ? "📄 PDF File" : "📎 Saved File";
      card.appendChild(fileBox);
    }

    const name = document.createElement("div");
    name.className = "diary-file-name";
    name.innerText = photo.name || "saved-file";
    ...
    actionWrap.className = "diary-file-actions";

    const download = document.createElement("a");
    download.href = photo.src;
    download.download = photo.name || "saved-file";
    download.className = "mini-btn diary-download-btn";
...
/* =========================================================
   SAVED CHATS PASSWORD PROTECTION
========================================================= */
function getSavedChatsPassword(){
  return localStorage.getItem("savedChatsPassword") || "";
}

function setSavedChatsPassword(){
  const oldPass = getSavedChatsPassword();

  if(oldPass){
    const verify = prompt("Enter old Saved Chats password:");
    if(verify !== oldPass){
  ...
  const p1 = prompt("Set new Saved Chats password:");
  if(!p1 || p1.trim().length < 3){
    alert("Password at least 3 characters ka rakho");
    return;
  }

  const p2 = prompt("Confirm new Saved Chats password:");
  if(p1 !== p2){
    alert("Passwords match nahi ho rahe");
    return;
  }

  localStorage.setItem("savedChatsPassword", p1);
  alert("Saved Chats password set ho gaya 💛");
}

function checkSavedChatsPassword(){
  const savedPass = getSavedChatsPassword();

  if(!savedPass){
    const setNow = confirm("Saved Chats ka password abhi set nahi hai. Kya abhi set karna hai?");
    if(setNow){
      setSavedChatsPassword();
      return !!getSavedChatsPassword();
    }
    return false;
  }

  const entered = prompt("Enter Saved Chats password:");
  return entered === savedPass;
}

function requireSavedChatAccess(callback){
  if(checkSavedChatsPassword()){
    callback();
  } else {
    alert("Saved Chats locked hain.");
  }
}
function renderSavedChatsList(){
  ...
  tools.className = "saved-chat-lock-tools";

  const setPassBtn = document.createElement("button");
  setPassBtn.className = "saved-chat-mini";
  setPassBtn.innerText = getSavedChatsPassword() ? "Change Saved Chats Password" : "Set Saved Chats Password";
  setPassBtn.onclick = () => setSavedChatsPassword();

  tools.appendChild(setPassBtn);
  list.appendChild(tools);

  if(chats.length === 0){
    const empty = document.createElement("div");
    empty.className = "mood-entry";
  ...
  lockedInfo.className = "mood-entry";
  lockedInfo.innerText = "Saved chats are locked. Open / rename / delete ke liye password lagega.";
  list.appendChild(lockedInfo);
  ...
  chats.forEach(chat => {
    ...
    openBtn.onclick = () => requireSavedChatAccess(() => openSavedChat(chat.id));
    ...
    renameBtn.innerText = "Rename";
    renameBtn.onclick = () => requireSavedChatAccess(() => renameSavedChat(chat.id));

    const deleteBtn = document.createElement("button");
    deleteBtn.className = "saved-chat-mini saved-chat-danger";
    deleteBtn.innerText = "Delete";
    deleteBtn.onclick = () => requireSavedChatAccess(() => deleteSavedChat(chat.id));
...
if(!getSavedChatsPassword()){
  const setNow = confirm("Saved chat saved. Kya Saved Chats ke liye password set karna hai?");
  if(setNow) setSavedChatsPassword();
}
...
function getTypingDelay(text, reply = ""){
  ...
  let delay = 400 + Math.min(outputLen * 8, 1200);

  if(inputLen > 60) delay += 300;
  if(inputLen > 120) delay += 300;

  const heavyWords = [
    "sad","cry","crying","low","alone","lonely","hurt",
    "stress","stressed","anxiety","panic","scared",
    "udaas","dukhi","rona","tension","dar","darr",
    "heavy","too much","broken"
  ];

  const lower = input.toLowerCase();
  const isHeavy = heavyWords.some(w => lower.includes(w));

  if(isHeavy){
    delay += 500;
  }

  const randomExtra = Math.floor(Math.random() * 400);

  return Math.min(delay + randomExtra, 2500);
}
...
function getRealisticTypingDelay(userText){
  ...
  const randomExtra = Math.floor(Math.random() * 450);

  return Math.min(delay + randomExtra, 2400);
}
...
function setupSmoothInputHandlers(){
  ...
  optimizedTyping(() => {
    ...
    if (chatInput.offsetHeight !== newHeight) {
  ...
  if(diaryText){
    diaryText.addEventListener("input", () => {
      ...
      diarySaveTimer = setTimeout(() => {
        if(currentDiaryFolder){
          localStorage.setItem("diary_" + currentDiaryFolder, diaryText.value);
        }
      }, 1000);
    });
  }
...
function setupPerformanceMode(){
  ...
  const heavyNodes = document.querySelectorAll(
    ".rain, .stars, .moon, .breath-circle, .mood-scene-stage, .video-card, .memory-video, section"
  );
  ...
  window.addEventListener("scroll", () => {
    ...
    clearTimeout(window.__scrollStopTimer);
    window.__scrollStopTimer = setTimeout(() => {
...
window.addEventListener("beforeunload", () => {
  try{
    ...
    const diaryText = document.getElementById("diaryText");
    if(currentDiaryFolder && diaryText){
      localStorage.setItem("diary_" + currentDiaryFolder, diaryText.value);
    }
...
function togglePerformanceMode(){
  ...
  if(body.classList.contains("low-performance")){
    localStorage.setItem("perfMode","on");
    btn.innerText = "⚡ Smooth Mode: ON";
  }else{
    localStorage.setItem("perfMode","off");
    btn.innerText = "⚡ Smooth Mode: OFF";
  }
}
...
window.addEventListener("load", () => {
  ...
  if(saved === "on"){
    ...
    if(btn) btn.innerText = "⚡ Smooth Mode: ON";
  }
});
/* =========================================================
   AUTO LOCK SYSTEM
   User inactive rahe to app automatically lock ho jayegi
========================================================= */
...
function resetAutoLockTimer(){
  ...
  // Agar app open hi nahi hai, timer mat chalao
...
function autoLockNow(){
  ...
  // Save important data before locking
  try{
    autoSaveNotes();
  ...
  // Main app hide, lock screen show
  if(main) main.style.display = "none";
  ...
  alert("For privacy, the website was auto-locked.");
}

function startAutoLockSystem(){
...
function loadDailyAffirmation(){
  const textEl = document.getElementById("dailyAffirmationText");
  if(!textEl) return;
  ...
  localStorage.setItem("dailyAffirmationDate", todayKey);

  textEl.innerText = random;
}

// load on start
window.addEventListener("load", loadDailyAffirmation);

/* =========================================================
   PHASE 3 — THEME SELECTOR
========================================================= */

function setCalmTheme(theme){
  ...
  localStorage.setItem("calmTheme", theme);
}
/* =========================================================
   PHASE 3 — JUST SIT MODE
========================================================= */

let justSitTimer = null;
let justSitSeconds = 60;

function openJustSitMode(){
  const overlay = document.getElementById("justSitOverlay");
  const timer = document.getElementById("justSitTimer");
  const line = document.getElementById("justSitLine");

  justSitSeconds = 60;

  if(overlay) overlay.style.display = "flex";
  if(timer) timer.innerText = "60 seconds of quiet";
  if(line) line.innerText = "You do not need to do anything right now.";

  clearInterval(justSitTimer);

  justSitTimer = setInterval(() => {
    justSitSeconds--;

    if(timer){
      timer.innerText = `${justSitSeconds} seconds of quiet`;
    }

    if(line){
      if(justSitSeconds <= 45 && justSitSeconds > 30){
        line.innerText = "Let your shoulders drop a little.";
      } else if(justSitSeconds <= 30 && justSitSeconds > 15){
        line.innerText = "Nothing needs to be solved in this minute.";
      } else if(justSitSeconds <= 15 && justSitSeconds > 0){
        line.innerText = "Just breathe. You are here.";
      }
    }

    if(justSitSeconds <= 0){
      clearInterval(justSitTimer);
      if(timer) timer.innerText = "You can stay longer if you want.";
      if(line) line.innerText = "You did enough by pausing.";
    }
  }, 1000);
}

function closeJustSitMode(){
  const overlay = document.getElementById("justSitOverlay");
  if(overlay) overlay.style.display = "none";
...
const habitItems = [
  ...
  { id:"stretch", icon:"🫶", text:"Relax shoulders / stretch" },
  { id:"focus", icon:"📝", text:"Study or focus for a little while" },
  { id:"sleep", icon:"😴", text:"Protect sleep / rest" },
  { id:"calm", icon:"🌬️", text:"Take one calm breathing break" }
];

function getHabitDateKey(){
  return "habits_" + getTodayDate();
}

function getTodayHabits(){
  return JSON.parse(localStorage.getItem(getHabitDateKey())) || {};
}

function saveTodayHabits(data){
  localStorage.setItem(getHabitDateKey(), JSON.stringify(data));
}

function renderHabitTracker(){
  const list = document.getElementById("habitList");
  const progress = document.getElementById("habitProgressText");

  if(!list) return;

  const saved = getTodayHabits();
  list.innerHTML = "";

  let doneCount = 0;

  habitItems.forEach(habit => {
    if(saved[habit.id]) doneCount++;

    const row = document.createElement("div");
    row.className = "habit-item" + (saved[habit.id] ? " done" : "");
  ...
  if(progress){
    progress.innerText = `${doneCount}/${habitItems.length} habits done today`;
  }
renderHabitWeekReport();
}

function toggleHabit(id){
  const saved = getTodayHabits();
  saved[id] = !saved[id];
  saveTodayHabits(saved);
  renderHabitTracker();
}

function resetTodayHabits(){
  const ok = confirm("Reset today's habits?");
  if(!ok) return;

  localStorage.removeItem(getHabitDateKey());
  renderHabitTracker();
}
window.addEventListener("load", renderHabitTracker);
...
function getHabitDateString(offsetDays){
  const d = new Date();
...
function getHabitAdvice(score){
  const lowAdvice = [
    "Is week thoda slow raha. No pressure — kal se restart karo.",
    "Ek habit se start karo. Sab ek saath nahi.",
    "Small steps = real progress.",
    "Consistency time leti hai, tension mat lo."
  ];

  const mediumAdvice = [
    "Good progress. Next week 1 habit aur add karo.",
    "Aap rhythm me aa rahe ho, bas continue rakho.",
    "Thoda aur push karoge to strong routine ban jayega.",
    "Ab thoda discipline add karo."
  ];

  const highAdvice = [
    "Great work 💛",
    "Consistency strong hai. Proud moment.",
    "Amazing routine build ho raha hai.",
  ...
  if(score <= 25){
    return mediumAdvice[Math.floor(Math.random() * mediumAdvice.length)];
  }

  return highAdvice[Math.floor(Math.random() * highAdvice.length)];
}

function renderHabitWeekReport(){
  const graph = document.getElementById("habitWeekGraph");
  const advice = document.getElementById("habitWeekAdvice");

  if(!graph || !advice) return;

  graph.innerHTML = "";

  let totalDone = 0;
  const maxPossible = habitItems.length * 7;

  for(let i = 6; i >= 0; i--){
    const date = getHabitDateString(i);
    const data = JSON.parse(localStorage.getItem("habits_" + date)) || {};

    let done = 0;

    habitItems.forEach(habit => {
      if(data[habit.id]) done++;
    });

    totalDone += done;

    const bar = document.createElement("div");
    bar.style.height = Math.max(10, (done / habitItems.length) * 100) + "%";
    bar.style.width = "12%";
  ...
  advice.innerText =
    `Last 7 days: ${totalDone}/${maxPossible}. ` + getHabitAdvice(totalDone);
}

/* =========================================================
   QUIET STRANGER 2 SCREEN UI
   Existing chat logic same rahega
========================================================= */

function chooseStrangerMode(mode, btn){
  setMode(mode, btn);

  const startScreen = document.getElementById("strangerStartScreen");
  const chatScreen = document.getElementById("strangerChatScreen");
  const modeView = document.getElementById("chatModeView");
  ...
  if(modeView){
    modeView.innerText = "Mode: " + (modeNames[mode] || "Just listen");
  }
